...
async function applyTransparency(enabled: boolean, radius: number): Promise<NativeTransparencyResult> {
  if (!HAS_TAURI) {
  ...
  try {
    ...
    if (os === "macos") {
      ...
      await invoke("set_desktop_blur", { radius: enabled ? radius : 0 });
...
export function setNativeTransparency(enabled: boolean, radius: number): Promise<NativeTransparencyResult> {
  const result = pending.then(() => applyTransparency(enabled, radius));
