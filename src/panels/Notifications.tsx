...
function NotificationToggle({ label, hint, checked, onChange }: { label: string; hint?: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <div className="settings-row">
    <div className="settings-row-copy"><span>{label}</span>{hint ? <small>{hint}</small> : null}</div>
    <button type="button" className={`switch ${checked ? "switch-on" : ""}`} role="switch" aria-label={label} aria-checked={checked} onClick={() => onChange(!checked)}><span className="switch-knob" /></button>
  </div>;
}
...
export function NotificationsSettings({ page }: { page: "activity" | "preferences" }) {
  ...
  const setDesktop = (enabled: boolean) => {
    if (!enabled) { configure({ desktop: false }); setPermissionError(""); return; }
    if (typeof Notification === "undefined") { setPermissionError("System notifications are unavailable in this runtime. Activity history still works."); return; }
    void Notification.requestPermission().then(permission => {
      configure({ desktop: permission === "granted" });
      setPermissionError(permission === "granted" ? "" : "Allow notifications in your browser or system settings first.");
    }).catch(() => setPermissionError("System notification permission could not be requested."));
  };
  return <div className="settings-page notification-settings">
    {page === "preferences" ? <>
      <section className="settings-section" aria-label="Delivery">
        ...
        <NotificationToggle label="System alerts" hint="Notify you while the app is unfocused. Activity details stay private." checked={preferences.desktop} onChange={setDesktop} />
      ...
      <section className="settings-section" aria-label="Activity categories">
        ...
        <details className="settings-disclosure"><summary>Individual events</summary><div className="notification-event-options settings-disclosure-body">
          {(Object.entries(NOTIFICATION_EVENTS) as [NotificationEvent, string][]).map(([event, label]) => <label key={event}><input type="checkbox" checked={preferences.events[event] !== false} onChange={change => configure({ events: { ...preferences.events, [event]: change.target.checked } })} /><span>{label}</span></label>)}
        </div></details>
      ...
      {filtered.length ? <Rows items={filtered.slice(0, limit)} /> : <div className="settings-empty"><Bell size={22} /><strong>{unreadOnly ? "No unread activity" : "No activity here yet"}</strong><p>{category === "all" ? "New notifications will appear here." : "Try another category to see more activity."}</p></div>}
