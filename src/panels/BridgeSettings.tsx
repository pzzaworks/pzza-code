...
import { Copy, ShieldCheck, Plus, X, RefreshCw, Loader2, Save, Ban } from "lucide-react";
...
export function BridgeSettings({ active = true }: { active?: boolean }) {
  ...
  return <section className="bridge-settings">
    <div className="bridge-heading"><div><h3><ShieldCheck size={18} /> Device bridge</h3><p>Let paired devices work on approved projects through MCP.</p></div>
    ...
    <p className="bridge-notice">Off by default. Disabling the bridge revokes access and cancels bridge jobs. Builds, terminal control, and UI tests execute code as this device’s user. Use a dedicated OS account for stronger isolation; these grants do not restrict SSH login itself.</p>
    ...
    {!state || !draft ? <AsyncButton className="btn" loading={busy || (!state && !error)} icon={RefreshCw} onClick={() => { setPendingAction("connect"); setBusy(true); void fetchBridgeState().then(value => { setState(value); setDraft(value.config); setJobs(value.jobs); setAudit(value.audit ?? []); setError(null); }).catch(report).finally(() => { setBusy(false); setPendingAction(null); }); }}>{error ? "Retry connection" : "Connect to device agent"}</AsyncButton> : <>
      <div className="bridge-card"><h4>This device’s identity</h4><code className="bridge-fingerprint">{state.identity.id}</code><p>Copy this public identity to the other device. Compare the full fingerprint through a trusted channel. No private key is shared.</p><AsyncButton className="btn btn-sm" loading={pendingAction === "copy"} disabled={busy} icon={Copy} iconSize={13} onClick={() => { setPendingAction("copy"); setBusy(true); void navigator.clipboard.writeText(JSON.stringify(state.identity)).then(() => setNote("Public device identity copied.")).catch(report).finally(() => { setBusy(false); setPendingAction(null); }); }}>Copy pairing identity</AsyncButton></div>
      <div className="bridge-card"><h4>Approved projects on this device</h4>
        ...
        <div className="bridge-form"><label>Project ID<input value={projectId} onChange={e => setProjectId(e.target.value)} placeholder="my-app" /></label><label>Absolute project folder<input value={root} onChange={e => setRoot(e.target.value)} placeholder="/Users/berke/projects/my-app" /></label></div>
      ...
      <div className="bridge-card"><h4>Pair a device</h4><p>Add identities on both devices. Permissions below grant incoming access to this device; the other device controls its own grants.</p>
        <div className="bridge-form"><label>Device name<input value={label} onChange={e => setLabel(e.target.value)} placeholder="MacBook" /></label><label>Verified SSH alias<input value={host} onChange={e => setHost(e.target.value)} placeholder="macbook" /></label></div>
        <label className="bridge-field">Public pairing identity<textarea value={pairing} onChange={e => setPairing(e.target.value)} rows={3} spellCheck={false} /></label>
      ...
      {draft.peers.map(peer => <div className="bridge-card" key={peer.id}>
        <div className="bridge-heading"><h4>{peer.label}</h4><label className="bridge-check"><input type="checkbox" checked={peer.enabled} onChange={e => updatePeer(peer.id, { enabled: e.target.checked })} /> Allow bridge access</label></div>
        <code className="bridge-fingerprint">{peer.id}</code>
        <div className="bridge-form"><label>SSH alias<input value={peer.host} onChange={e => updatePeer(peer.id, { host: e.target.value })} /></label><label>Agent port<input type="number" min={1024} max={65535} value={peer.port} onChange={e => updatePeer(peer.id, { port: Number(e.target.value) })} /></label></div>
      ...
      <div className="bridge-save"><span>Project and permission edits apply when saved.</span><AsyncButton className="btn btn-accent" loading={pendingAction === "save"} icon={Save} disabled={busy || JSON.stringify(draft) === JSON.stringify(state.config)} onClick={() => void save(draft)}>Save bridge settings</AsyncButton></div>
      <div className="bridge-card"><h4>Jobs and submission approvals</h4><p>Closing this panel keeps jobs running. Approvals are only available here, never through remote MCP.</p>
