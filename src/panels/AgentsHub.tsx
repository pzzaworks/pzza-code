...
function AgentsHubLibrary({ active, section: tab, onSectionChange, onOpenSession }: Omit<AgentsHubProps, "section"> & { section: Exclude<AgentsHubSection, "instructions"> }) {
  ...
  const add = () => {
    const id = uid();
    edit(value => tab === "skills" ? { ...value, skills: [...value.skills, { id, name: "New skill", content: "" }] }
  ...
  const list = state ? (tab === "skills" ? state.skills : state.profiles).filter(item => item.name.toLowerCase().includes(query.toLowerCase())) : [];
