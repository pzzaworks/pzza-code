...
import { fetchHub, saveHub, previewHub, applyHub, importHubSkill, discoverHubInstructions, previewHubInstruction, type HubDiscovery, type HubState, type HubProfile, type HubPreview } from "../agentsHubApi";
...
export function AgentsHubContent({ active, section: tab, onSectionChange, onOpenSession }: {
  ...
  const refresh = async () => { if (refreshing.current) return; refreshing.current = true; setPendingAction("refresh"); setBusy(true); setError(""); try {
    if (tab === "instructions" && devices.length) {
      const found = await discoverHubInstructions(devices.map(device => ({ host: deviceHost(device), name: device.name })));
      setDiscovery(found); setState(found.state);
      const failures = found.devices.filter(device => device.error).map(device => `${device.name}: ${device.error}`);
      if (failures.length) setError(failures.join("; "));
    } else setState(await fetchHub());
    setDirty(false); setPreviews([]); } catch (cause) { report(cause); } finally { refreshing.current = false; setBusy(false); setPendingAction(null); } };
  useEffect(() => { if (active && !dirty) void refresh(); }, [active, tab, devices]);
  useEffect(() => {
