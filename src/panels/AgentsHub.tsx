...
import { Bot, BookOpen, FileText, Layers, Play, Plus, RefreshCw, Save, Search, ArrowLeft, ChevronRight, X, FileUp, FolderSync } from "lucide-react";
...
import { fetchHub, saveHub, previewHub, applyHub, importHubSkill, discoverHubInstructions, readHubInstruction, previewHubInstruction, type HubDiscovery, type HubDiscoveredFile, type HubDiscoveryDevice, type HubState, type HubProfile, type HubPreview } from "../agentsHubApi";
import { SKILL_CATALOG } from "../agentsHubCatalog";
import "./AgentsHub.css";

export type AgentsHubSection = "instructions" | "skills" | "discover" | "profiles" | "deployments";
export const agentsHubSections = [{ id: "instructions", label: "Instructions", icon: FileText }, { id: "skills", label: "My skills", icon: BookOpen }, { id: "discover", label: "Discover skills", icon: Search }, { id: "profiles", label: "Agent profiles", icon: Bot }, { id: "deployments", label: "Deployments", icon: Layers }] as const;
...
export function AgentsHubContent({ active, section: tab, onSectionChange, onOpenSession }: {
  active: boolean;
  ...
  const [discovery, setDiscovery] = useState<HubDiscovery | null>(null);
  const [discovering, setDiscovering] = useState(false);
  const [discoveryError, setDiscoveryError] = useState("");
  const [deployDocument, setDeployDocument] = useState<string | null>(null);
  ...
  const deploying = Boolean(deployProfile || deployDocument);
  const currentDocument = state?.documents.find(document => document.id === deployDocument);
  const currentProfile = state?.profiles.find(profile => profile.id === deployProfile);
  const discover = async () => {
    if (discovering || !devices.length) return;
    setDiscovering(true); setDiscoveryError("");
    let root = "~/Projects";
    try { root = localStorage.getItem("pzza.projectsRoot") || root; } catch { /* Use the configured default when storage is unavailable. */ }
    try { setDiscovery(await discoverHubInstructions(root, devices.map(device => ({ host: deviceHost(device), name: device.name })))); }
    catch (cause) { setDiscoveryError(cause instanceof Error ? cause.message : "Instruction discovery failed"); }
    finally { setDiscovering(false); }
  };
  useEffect(() => { if (active && tab === "instructions") void discover(); }, [active, tab, devices]);
  const importDiscovered = async (device: HubDiscoveryDevice, file: HubDiscoveredFile) => {
    if (busy || !device.root) return;
    setBusy(true); setPendingAction(`instruction:${device.host}:${file.path}`); setError("");
    try {
      const document = await readHubInstruction(device.host, device.root, file);
      const id = uid();
      setState(value => value ? { ...value, documents: [...value.documents, { ...document, id }] } : value);
      setSelected(id); setDirty(true); setPreviews([]); setRendered(false);
    } catch (cause) { report(cause); }
    finally { setBusy(false); setPendingAction(null); }
  };
  const addAllDevices = () => {
    const first = targets[0];
    const sourceRoot = discovery?.devices.find(device => device.host === first?.host)?.root;
    const relative = sourceRoot && first?.cwd.startsWith(sourceRoot + "/") ? first.cwd.slice(sourceRoot.length) : first?.cwd === sourceRoot ? "" : null;
    setTargets(devices.map(device => {
      const host = deviceHost(device);
      const existing = targets.find(target => target.host === host);
      const root = discovery?.devices.find(item => item.host === host)?.root;
      return existing ?? { host, cwd: root && relative !== null ? root + relative : "" };
    }));
    setPreviews([]);
  };
  ...
  useEffect(() => { if (active && !state) void refresh(); }, [active]);
  useEffect(() => {
    if (!active) { setPicker(null); setImportInstructions(false); }
  }, [active]);
  ...
  useEffect(() => {
    ...
    setDeployProfile(null); setDeployDocument(null); setPreviews([]); setPicker(null);
  }, [tab]);
  const add = () => {
    const id = uid();
  ...
  const startPreview = async () => {
    if ((!deployProfile && !deployDocument) || dirty) return;
    setPendingAction("preview"); setBusy(true); setError(""); setPreviews([]);
    try {
      const plans: HubPreview[] = [];
      for (const target of targets) plans.push(await (deployDocument ? previewHubInstruction(deployDocument, target.host, target.cwd, adoptExisting) : previewHub(deployProfile!, target.host, target.cwd, adoptExisting)));
  ...
  const apply = async (mode: "sync" | "deploy") => {
    ...
    try {
      ...
      setDeployProfile(null); setDeployDocument(null); onSectionChange("deployments");
  ...
  const itemDetail = (id: string) => {
    ...
    if (tab === "instructions" && document) return state?.frameworks.find(item => item.id === document.framework)?.label ?? document.framework;
      ...
      <div className="ah-layout">
        <main className="ah-main"><div className="ah-toolbar"><span className="ah-save-status" role="status">{dirty ? "Unsaved changes" : busy ? "Working…" : "Local library"}</span><AsyncButton className="btn btn-sm" loading={busy && refreshing.current && !dirty} disabled={busy || dirty} title="Refresh library" aria-label="Refresh library" icon={RefreshCw} onClick={() => void refresh()} />{dirty ? <AsyncButton className="btn btn-sm" loading={busy && refreshing.current && dirty} disabled={busy} icon={RefreshCw} onClick={() => void refresh()}>Discard</AsyncButton> : null}<AsyncButton className="btn btn-sm" loading={pendingAction === "save"} icon={Save} disabled={busy || !dirty} onClick={() => void save()}>Save changes</AsyncButton></div>
          ...
          {!state ? <div className="ah-message">{busy ? "Loading library…" : "The library is unavailable. Use Refresh to retry."}</div> : <div className="ah-scroll">
            {deploying ? <section className="ah-deploy"><div className="ah-actions"><h3>{deployDocument ? "Sync" : "Deploy"} {currentDocument?.name ?? currentProfile?.name}</h3><button className="btn btn-sm" disabled={busy} onClick={() => { setDeployProfile(null); setDeployDocument(null); setPreviews([]); }}>Cancel</button></div>
            ...
            <div hidden={deploying}>{tab === "discover" ? <><input className="input ah-search" aria-label="Search skill catalog" placeholder="Search skills…" value={query} onChange={event => { setQuery(event.target.value); setLimit(12); }} /><div className="ah-catalog">{catalog.slice(0, limit).map(item => <article key={item.id}><div className="ah-catalog-copy"><h3>{item.name}</h3><p>{item.description}</p><small><a onClick={visitSource} href={item.licenseUrl} target="_blank" rel="noopener noreferrer">{item.license}</a> · {item.stars === null ? "Stars unavailable" : `${item.stars.toLocaleString()} repository stars`} · checked {item.checkedAt}</small><a onClick={visitSource} href={`${item.sourceUrl}/tree/HEAD/${item.subpath}`} target="_blank" rel="noopener noreferrer">Source: {item.sourceUrl.replace("https://github.com/", "")}</a></div><AsyncButton className="btn btn-sm" loading={pendingAction === `import:${item.id}`} disabled={busy || dirty} onClick={() => {
              ...
              }}>Import</AsyncButton></article>)}</div><ScrollMore hasMore={limit < catalog.length} loadMore={() => setLimit(value => value + 12)} /></> : tab === "deployments" ? <>{state.deployments.length ? [...state.deployments].reverse().slice(0, limit).map(deployment => <article className="ah-history" key={deployment.id}><div className="ah-history-head"><strong>{state.profiles.find(profile => profile.id === deployment.profileId)?.name ?? deployment.profileId}</strong><span className="ah-status">{deployment.status}</span></div><p className="ah-history-path">{deployment.cwd}</p><small>{deployment.host || "This device"}</small><small>{new Date(deployment.createdAt).toLocaleString()} · {deployment.mode}</small>{deployment.backupPath ? <p className="set-note">Previous files backed up on this device: <code>{deployment.backupPath}</code></p> : null}{deployment.error ? <p role="alert">{deployment.error}</p> : null}{deployment.session ? <button className="btn btn-sm" onClick={() => { useStore.getState().openSession(deployment.session!, deployment.cwd, deployment.host); onOpenSession(); }}>Open session</button> : null}</article>) : <p className="set-note">No deployments yet. Create a profile, then choose Deploy.</p>}<ScrollMore hasMore={limit < state.deployments.length} loadMore={() => setLimit(value => value + 12)} /></> : <div className="ah-workbench"><div className="ah-library" hidden={Boolean(selected)}><div className="ah-library-toolbar"><input className="input" aria-label="Search library" placeholder="Search…" value={query} onChange={event => setQuery(event.target.value)} /><button className="btn btn-sm" disabled={busy} onClick={add}><Plus size={14} />{tab === "profiles" ? "New profile" : tab === "skills" ? "New skill" : "New instructions"}</button>{tab === "instructions" ? <AsyncButton className="btn btn-sm" loading={pendingAction === "import-instructions"} icon={FileUp} disabled={busy} onClick={() => setImportInstructions(true)}>Import file</AsyncButton> : null}</div>{tab === "instructions" ? <details className="ah-discovery" open={list.length === 0}><summary>Detected on devices {discovering ? "· scanning…" : ""}</summary><div className="ah-actions"><AsyncButton className="btn btn-sm" loading={discovering} icon={RefreshCw} onClick={() => void discover()}>Scan devices</AsyncButton></div>{discoveryError ? <p role="alert" className="set-note">{discoveryError}</p> : null}{discovery?.devices.map(device => <div key={device.host}><small>{device.name}</small>{device.error ? <p className="set-note" role="alert">{device.error}</p> : null}{device.truncated ? <p className="set-note">Scan limit reached. Choose a narrower project root to find more files.</p> : null}{!device.error && !device.files.length ? <p className="set-note">No instruction files found in this project root.</p> : null}{device.files.map(file => <div className="ah-discovered-file" key={file.path}><span><strong>{file.path}</strong><small>{state.frameworks.find(framework => framework.id === file.framework)?.label} · {file.bytes.toLocaleString()} bytes</small></span><AsyncButton className="btn btn-sm" icon={FileUp} loading={pendingAction === `instruction:${device.host}:${file.path}`} disabled={busy || state.documents.length >= 100} onClick={() => void importDiscovered(device, file)}>Import</AsyncButton></div>)}</div>)}</details> : null}<div className="ah-library-list">{list.map(item => <button className={`ah-list-item ${selected === item.id ? "active" : ""}`} key={item.id} onClick={() => { setSelected(item.id); setRendered(false); }}><span className="ah-item-copy"><strong>{item.name}</strong><small>{itemDetail(item.id)}</small></span><ChevronRight size={14} /></button>)}</div>{!list.length ? <p className="ah-empty">{query ? "No matching items." : "Your library is empty. Create an item to get started."}</p> : null}</div><section className="ah-editor" hidden={!selected}><button className="ah-back" onClick={() => setSelected("")}><ArrowLeft size={14} />All {agentsHubSections.find(item => item.id === tab)?.label.toLowerCase()}</button>
              {editedDocument ? <><div className="ah-fields"><label>Name<input className="input" value={editedDocument.name} disabled={busy} onChange={event => edit(value => tab === "instructions" ? { ...value, documents: value.documents.map(item => item.id === selected ? { ...item, name: event.target.value } : item) } : { ...value, skills: value.skills.map(item => item.id === selected ? { ...item, name: event.target.value } : item) })} /></label>
                {tab === "instructions" && selectedDocument ? <label>Framework<Select value={selectedDocument.framework} options={state.frameworks.map(framework => ({ value: framework.id, label: framework.label, sub: framework.instructionFile }))} onChange={framework => edit(value => ({ ...value, documents: value.documents.map(item => item.id === selected ? { ...item, framework } : item) }))} /></label> : <p className="set-note">SKILL.md content. Use the skill format expected by your selected framework.{selectedSkill?.files ? ` This imported bundle contains ${selectedSkill.files.length} files.` : ""}</p>}</div>
                ...
                {tab === "instructions" && selectedDocument ? <button className="btn btn-sm" disabled={busy || dirty} onClick={() => { setDeployDocument(selectedDocument.id); setDeployProfile(null); setTargets([{ host: "", cwd: "" }]); setPreviews([]); setAdoptExisting(false); }}><FolderSync size={14} />Sync instruction to devices</button> : null}
