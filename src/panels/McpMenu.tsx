    ...
    <div className="settings-page mcp-settings">
      ...
      <section className="settings-section" aria-label="Integration health">
        ...
        {devices.map(device => {
          ...
          return <details key={device.id} className="settings-disclosure"><summary>{device.name} · {value?.checking ? "Checking…" : value?.error ? "Unavailable" : value?.results.some(result => result.status === "unresolved") ? "Needs attention" : value?.results.some(result => result.status === "repaired") ? "Repaired" : value?.results.length ? "Ready" : "No servers configured"}</summary>
