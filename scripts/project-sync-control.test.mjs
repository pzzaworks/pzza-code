...
test('sync summaries distinguish safe skips and current repositories from unsynced work', () => {
  ...
  assert.equal(stashed.needsAttention, false);
  assert.match(stashed.body, /local changes preserved/);
  const dirtyOutcome = result(['dirty']).devices[0].results[0];
  const failedOutcome = result(['failed']).devices[0].results[0];
  assert.equal(control.hasUnresolvedProjectSync(dirtyOutcome, []), true, 'the Attention filter must retain dirty enabled rows');
  assert.equal(control.hasUnresolvedProjectSync(failedOutcome, []), true);
  assert.equal(control.hasUnresolvedProjectSync(result(['stashed']).devices[0].results[0], []), false);
  assert.equal(control.hasUnresolvedProjectSync(undefined, [{ status: 'failed' }]), true);
