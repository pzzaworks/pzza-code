...
const skill = { type: "object", additionalProperties: false, required: ["id", "name", "content"], properties: { id, name: text, content: text, sourceUrl: text, license: text, commit: text, files: { type: "array", maxItems: 100, items: asset } } };
...
const tool = (name, description, endpoint, properties, required, readOnly = false) => ({
  ...
  run: async (args) => {
    ...
    if (["save", "import-skill"].includes(endpoint)) return localGet("/agents-hub/summary");
...
export const AGENTS_HUB_TOOLS = [
  ...
  tool("agents_hub_update", "Merge one library item against the current revision. Omitted skill files remain intact; an explicit files array replaces the bundle. Pass only fields being edited, not the asset metadata returned by agents_hub_get.", "update", {
    revision: { type: "integer", minimum: 0 }, kind: { type: "string", enum: ["document", "skill", "profile"] },
    item: { type: "object", additionalProperties: false, required: ["id"], properties: { id, name: text, framework, content: text, systemPrompt: text, instructionIds: { type: "array", items: id }, skillIds: { type: "array", items: id }, sourceUrl: text, license: text, commit: text, files: { type: "array", maxItems: 100, items: asset } } },
  }, ["revision", "kind", "item"]),
  ...
  tool("agents_hub_import_skill", "Import a complete public GitHub skill directory and its assets pinned to a verified commit. Downloads source only and never executes it. Inspect its instructions, scripts and license before attaching to a profile.", "import-skill", { revision: { type: "integer", minimum: 0 }, sourceUrl: text, subpath: { type: "string", description: "Repository-relative directory containing SKILL.md" }, ref: { type: "string", description: "Optional branch, tag or commit to resolve to an immutable commit" } }, ["revision", "sourceUrl", "subpath"]),
];
