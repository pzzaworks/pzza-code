// pzza-console backend. Runs in one of two roles:
//
...
// Per-framework config snippets for adding the pzza-console MCP server.
function mcpConfigs(mcpPath) {
  const jsonEntry = { command: "node", args: [mcpPath] };
  const jsonBlock = (root) =>
    JSON.stringify({ [root]: { "pzza-console": jsonEntry } }, null, 2);
  return {
    frameworks: {
      ...
      codex: {
        label: "Codex",
        cli: true,
        config: `[mcp_servers.pzza-console]\ncommand = "node"\nargs = ["${mcpPath}"]`,
      },
      ...
      zed: {
        ...
        config: JSON.stringify(
          { context_servers: { "pzza-console": { command: { path: "node", args: [mcpPath] } } } },
          null,
          2,
        ),
      },
...
async function mcpInstall(framework, mcpPath) {
  if (framework === "claude") {
    const r = await run("claude", ["mcp", "add", "-s", "user", "pzza-console", "--", "node", mcpPath]);
    return { framework, ...r, via: "claude mcp add" };
  }
  if (framework === "codex") {
    const r = await run("codex", ["mcp", "add", "pzza-console", "--", "node", mcpPath]);
